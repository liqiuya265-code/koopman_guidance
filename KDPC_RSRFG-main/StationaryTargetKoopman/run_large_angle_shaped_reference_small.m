%% Small-amplitude shaped-reference check for gamma_f=60 deg
clear; clc; close all;
...
cases(1)=make_case('zero-tau-baseline','large_shape_small_baseline',0,1,0.6);
cases(2)=make_case('shape-pos-0p02','large_shape_small_pos_0p02',0.02,1,0.6);
cases(3)=make_case('shape-pos-0p05','large_shape_small_pos_0p05',0.05,1,0.6);
cases(4)=make_case('shape-pos-0p10','large_shape_small_pos_0p10',0.10,1,0.6);
cases(5)=make_case('shape-neg-0p02','large_shape_small_neg_0p02',0.02,-1,0.6);
cases(6)=make_case('shape-neg-0p05','large_shape_small_neg_0p05',0.05,-1,0.6);
cases(7)=make_case('shape-neg-0p10','large_shape_small_neg_0p10',0.10,-1,0.6);
...
for i=1:numel(cases)
    fprintf('\nRunning small shaped-reference case %d/%d: %s\n', ...
...
summaryPath=fullfile(resultsDir,'large_angle_shaped_reference_small_summary.csv');
write_shape_summary(summary,summaryPath);
fprintf('\nSmall shaped-reference summary saved to %s\n',summaryPath);
...
function write_shape_summary(summary,path)
    fid=fopen(path,'w');
    fprintf(fid,['case,amplitude,sign,satisfied,miss_m,angle_error_deg,', ...
    ...
    for i=1:numel(summary)
        fprintf(fid,'"%s",%.8f,%.8f,%d,%.8f,%.8f,%d,%.8f,%.8f,%d,%d\n', ...
            ...
            summary(i).satisfied,summary(i).miss_m, ...
