%% Large-impact-angle validation without hard input-rate constraint
clear; clc; close all;
...
for a=angles
    for g0=initialGammas
        idx=idx+1;
        fprintf('\nRunning large-angle no-du case %d/%d: gamma_f=%d deg, gamma_0=%d deg\n', ...
            idx,nCase,a,g0);
        ...
        rdOverride=1.5;
        resultSuffixOverride=sprintf('angle_only_g%d_i%s_nodu', ...
            a,signed_label(g0));
...
summaryPath=fullfile(resultsDir,'angle_only_large_angle_no_du_scenarios_summary.csv');
write_large_angle_summary(summary,summaryPath);
plot_large_angle_summary(summary, ...
    fullfile(resultsDir,'angle_only_large_angle_no_du_scenarios_summary.png'));
fprintf('\nLarge-angle no-du scenario summary saved to %s\n',summaryPath);
...
function plot_large_angle_summary(summary,path)
    ...
    title('Large-impact-angle validation, no hard du constraint');
