%% Hard acceleration-vibration constraint scan for gamma_f=60 deg
clear; clc; close all;
...
deltaAValues=10;%[inf,15,10,8,5,3]
...
for i=1:numel(deltaAValues)
    ...
    if isfinite(deltaA)
        ...
        suffix=sprintf('large60_zero_tau_dA_%g',deltaA);
    else
        label='inf';
        suffix='large60_zero_tau_dA_inf';
    end
    fprintf('\nRunning gamma_f=60 hard acceleration-vibration scan %d/%d: deltaA=%s m/s^2\n', ...
...
summaryPath=fullfile(resultsDir,'large_angle_60_accel_vibration_constraint_scan_summary.csv');
write_accel_scan_summary(summary,summaryPath);
plot_accel_scan_summary(summary,fullfile(resultsDir, ...
    'large_angle_60_accel_vibration_constraint_scan_summary.png'));
plot_acceleration_vibration_comparison(deltaAValues,resultsDir,fullfile(resultsDir, ...
    'large_angle_60_accel_vibration_comparison.png'));
fprintf('\nGamma_f=60 hard acceleration-vibration scan summary saved to %s\n',summaryPath);
...
function plot_accel_scan_summary(summary,path)
    ...
    title('Gamma_f=60 deg: hard acceleration-vibration constraint scan');
    ...
    xlabel('Delta A command max [m/s^2]');
    ylabel('max Delta A actual [m/s^2]');
