%% Gamma_f=60 deg agile check with 5 s horizon for comparison
clear; clc; close all;
...
resultSuffixOverride='large60_agile_dA_30_rd_0p2_h50';
run(fullfile(baseDir,'run_stationary_target_demo.m'));

summaryPath=fullfile(resultsDir,'large_angle_60_agile_accel_h50_check_summary.csv');
...
fprintf(fid,['"large-angle-60deg-agile-h50",%.8f,%.8f,%.8f,%d,%.8f,%.8f,%.8f,%.8f,%.8f,', ...
...
fprintf('\nGamma_f=60 agile h50 summary saved to %s\n',summaryPath);
